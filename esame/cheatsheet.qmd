...
- $\forall ~ binarySearch(A,v,i,j), \forall h \in A[1...i-1], h < v ~ \myand  ~ \forall k \in A[i...j], k \geq v$

La seconda invariante ci dice qualcosa di più: con questo algoritmo non troviamo un'occorrenza qualsiasi di v; bensì troviamo la **prima** occorrenza di v. (Con `ceil()` troveremmo l'ultima -- ma andrebbero modificati gli indici delle chiamate ricorsive).
Dimostriamo quindi la seconda invariante per induzione.

***Base:** con $i=1, j=n$ vale $A[i...j]=A$, dunque $A[1...0] = \emptyset = A[n...n+1]$ ed entrambe le condizioni sono soddisfatte*

***Passo induttivo:** Supponiamo che per $binarySearch(A,v,i,j)$ valga l'invariante e dimostriamo che vale anche per l'invocazione successiva, $binarySearch(A,v,i',j')$*

  *1) Se $i > j$ allora $A[i...j] = \emptyset$ e la funzione termina*  
  *2) Se $i = j$ allora $v \in A \iif v = A[i]$*  
  *Posto $m:=\left\lfloor (i+j)/2\right\rfloor$*  
...
*iii) $T(n) \leq (a_1+a_2)T(n/min(b_1, b_2)) \implies T(n) = O(...)$ per il MT*  
*iv) $T(n) \geq (a_1+a_2)T(n/max(b_1, b_2)) \implies T(n) = \Omega(...)$ per il MT*  
...
1) *Se $\exists \epsilon>0 : f(n)=O(n^{\log_{b}(a)-\epsilon})$*  
*allora $T(n)=\Theta(n^{\log_{b}{a}})$*  

2) *Se $f(n)= \Theta(n\log_b{a})$*  
*allora $T(n)=\Theta(f(n)\log{n})$*  

3) *Se $\exists \epsilon > 0 : f(n)= \Omega(n^{\log_{b}{a}+\epsilon}) ∧ \exists c : 0<c<1, \exists m \geq 0 : af(n/b) \leq cf (n) \forall n \geq m$*  
